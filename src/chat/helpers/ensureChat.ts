...
export async function ensureChat(
  ...
  const contact = ContactStore.get(wid) || (classicWid && ContactStore.get(classicWid));
  const widChat = ChatStore.get(wid);
  const classicChat = classicWid ? ChatStore.get(classicWid) : undefined;
  const prefersClassic =
    wid.isLid?.() &&
    classicWid &&
    (classicChat || (contact?.id?.server !== 'lid' && contact?.id?._serialized !== wid._serialized));
...
export function ensureChatSync(chatId: WidLike): ChatModel {
  ...
  const candidates = Array.from(
    new Set(
      [contact?.id, classicChat?.id, widChat?.id, classicWid, wid, contact?.lid].filter(
        (id) => id,
      ) as Wid[],
    ),
  );
