...
import { ChatStore, ContactStore, functions, WidFactory } from '../../whatsapp';
import {
  resolveCanonicalChatId,
  resolveCanonicalChatIdSync,
} from './resolveCanonicalChatId';
import { resolveCanonicalWid } from './resolveCanonicalWid';
...
function isLidMigrated(): boolean {
  try {
    ...
    return typeof migrated === 'function' ? !!migrated() : !!migrated;
  } catch {
    return false;
  }
}
...
export async function ensureChat(
  ...
  const canonicalChatId = await resolveCanonicalChatId(chatId);
  const canonical = resolveCanonicalWid(canonicalChatId);
  const wid = coerceWid(canonical as any);
  const migrated = isLidMigrated();

  // Fallback classic id for environments not migrated to LID. When a caller
  // passes a LID but the account still stores chats under @c.us/@g.us, we need
  // to reuse the existing chat instead of creating a duplicate LID entry.
  const classicWid =
    wid.isLid?.() && wid.user
      ? createWid(`${wid.user}@c.us`) || createWid(wid.user)
      : undefined;

  const contact =
    ContactStore.get(wid) || (classicWid && ContactStore.get(classicWid));
  const widChat = ChatStore.get(wid);
  const classicChat = classicWid ? ChatStore.get(classicWid) : undefined;
  ...
  const candidateIds = Array.from(
    new Set(
      [
        coerceWid(canonical as any),
        contact?.lid,
        lid,
        contact?.id,
        widChat?.id,
        classicChat?.id,
        // Always try the classic wid when the request came with a LID so we
        // reuse any existing chat on non-migrated accounts before attempting
        // to create or resolve a LID chat.
        classicWid,
        wid,
      ].filter((id) => id) as Wid[]
    )
  ...
  // In migrated sessions, always prefer the LID-backed chat when available to
  // avoid writing/reading from legacy @c.us entries that can lack the LID
  // metadata and trigger storage errors.
  if (ensureLid && migrated && !chat.id?.isLid?.()) {
    const targetLid = (
      lid?.isLid?.() ? lid : contact?.lid?.isLid?.() ? contact.lid : undefined
    ) as Wid | undefined;

    if (targetLid && targetLid._serialized !== chat.id.toString()) {
      const resolvedChat =
        tryGetChat(targetLid) ||
        (await assertFindChat(targetLid).catch((error) => {
          if (error instanceof InvalidChat) {
            lastInvalid = error;
            return undefined;
          }
  ...
  if (ensureLid && chat?.id?.isUser?.()) {
    let resolved = contact?.lid?.isLid?.() ? contact.lid : lid;

    if (!resolved) {
      resolved = await resolveChatLid(chat.id);
    }
...
export function ensureChatSync(chatId: WidLike): ChatModel {
  const canonical = resolveCanonicalWid(resolveCanonicalChatIdSync(chatId));
  const wid = coerceWid(canonical as any);
  const classicWid =
    wid.isLid?.() && wid.user
      ? createWid(`${wid.user}@c.us`) || createWid(wid.user)
  ...
  let lastError: InvalidChat | undefined;
  ...
  throw lastError ?? new InvalidChat(wid);
}
